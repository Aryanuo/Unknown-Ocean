...
const EnvironmentScene = React.memo(function EnvironmentScene({ biome, onScanCreature, onDiscovery, onInvestigateMystery, discoveredMysteryIds, onEventObjectInteract, onArtefactToast }: EnvSceneProps) {
  ...
  return (
    <>
      ...
      <ArtefactManager onCollectToast={onArtefactToast} />
      ...
      <UnderwaterAtmosphere biome={biome} />
      <BiomeDecor biome={biome} />
      <ArtifactPulse3D />
      <Waypoint3D />
    </>
  )
...
export default function OceanScene() {
  ...
  // Dev-only stats overlay (backtick ` to toggle)
  ...
  // ── ` key toggles dev stats ────────────────────────────────────────────────
  useEffect(() => {
    if (!import.meta.env.DEV) return
    const handler = (e: KeyboardEvent) => {
      if (e.key === '`' && !e.repeat) setShowStats(prev => !prev)
    }
